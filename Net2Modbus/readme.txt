...
20180815
58、在IAP_TFTP 中增加超时机制，如果发送文件为空则会占用一个udppcb，加入超时后在后续没有数据传输时
将其释放即可，解决56条里面的TFTP死机问题
，时间定死为250ms*85
20180901

20180903
59、debug到TCP重练出现无连接的情况，是因为connect连接函数没有成功，导致绑定成功的pcb驻留在绑定链表中，没有释放
而当他再次进行new PCB 资源空返回0000，然后绑定比失败，失败后自动返回，以此循环进入死锁状态。导致udp正常TCP
连接中断。这个问题进行了修改，
 if( tcp_connect(wpcb, &ip_addr, port_sever, tcp_client_connected)!=ERR_OK)
		  {

			  /*#20180903 使用udp 21228 端口狂发getrun,然后不断的重练的情况下发现
			  会出现死机的情况，20180903日debug到该问题。是因为connct链接返回失败
			  pcb没有正确的进入tcp_active_pcbs链表中。导致再次进行newD的时候占内存而失败
			  实际此时的pcb驻留在tcp_bound_pcbs，导致了后续bind失败从而返回。然后再次进入TCP new
			  再次分配失败再次bind失败，再次返回，一次往复就出现了死锁*/
 ...
 60、对59问题确认，TCP无连接问题确实是connect连接失败造成的，原因是ip_route(&(pcb->remote_ip));返回了null
 追寻下去 是下面这一段返回了NULL
 if ((netif_default == NULL) || (!netif_is_up(netif_default))) 
  ...
  然后验证上面加的代码	 ERR_Connect();处理正确发挥作用，释放了在bound链表中的PCB，这样再次下来就可以继续
  使用new PCB了。也就不存在死TCP链接的情况了。
  然后发现在TCP PCB恢复之后，设备还是不能正常工作，SYN可以发出，但是PC回复SYN ACK无回复，重启路由器后
  回复正常。这个问题和路由器很有关系，但是他暴漏了没有处理connect的bug.务必处理必要的返回
  这个问题早就在追踪一直没能解决，先是jlink-debug 失败-而后是极难复现。唯一可以复现的情况就是59提到的狂发UDP
...
发现如果只保持TCP链接，2天后收发只能发送getrun,其他指令无返回。bug正砸查找，应该是pbuf没有内存引起的。只能发送端数据，不能发送场数据

20180907

 实在不行，在发送中判断new不出来字节，直接reboot.
 正在追宗该问题
 20180913

debug没找到问题，直接在TCP链接中增加一个侦测，检测到new不出来就强制重启吧。
if( tcp_write(tpcb, &UID_STM32, 19, 1)==ERR_OK)判断是否成功，成功了就进行发送，否则就是出错，重启重新运行。
20180918