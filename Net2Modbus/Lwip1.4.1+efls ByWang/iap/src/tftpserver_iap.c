...
static void*Timeoutargs;
extern u8_t tftptimeoutTimer;//TFTP超时定时器
extern u8_t tftptimeout;//TFTP超时定时器

extern struct udp_pcb *Timeoutupcb; //超时PCB变量
extern struct ip_addr  Timeoutaddr;
extern u16_t           Timeoutport;
extern struct pbuf     *Timeoutportpkt_buf;
...
/**
  ...
  * @retval none
  */
  extern  void IAP_EndSetFlash(void);
static void IAP_wrq_recv_callback(void *_args, struct udp_pcb *upcb, struct pbuf *pkt_buf, struct ip_addr *addr, u16_t port)
{
  ...
  if (pkt_buf->len < TFTP_DATA_PKT_LEN_MAX)
  {
	 ...
	 //关闭定时器。
	 tftptimeoutTimer=0;//TFTP超时定时器
	 tftptimeout=0;//TFTP超时定时器
...
/**
  ...
  * @retval none
  */
 extern  uint32_t Flash_First_Page_Address ;
static int IAP_tftp_process_write(struct udp_pcb *upcb, struct ip_addr *to, int to_port)
{
  ...
  * therefore we can use regular malloc   */
  ...
  FLASH_If_Erase(Flash_First_Page_Address);
 
  Flash_Write_Address = Flash_First_Page_Address;
  ...
  if( tftptimeoutTimer)//如果开启超时定时器则进入
  {
     tftptimeout++;
           ...
           //  tftp_cleanup_rd(Timeoutupcb, Timeoutargs);
			  ...
			  IAP_tftp_cleanup_wr(Timeoutupcb, Timeoutargs);
...
void tftpiap_cleanup_rd(struct udp_pcb *upcb, tftp_connection_args *args)
{
